...
def local_noise(init_im, patch_size, sig=1, seed=None):
    '''
    - init_im (torch.Tensor or None): initial image
    - patch_size (int): size of the square patch where noise is added
    - sig (float): standard deviation of Gaussian noise
    '''

    if seed is not None: 
        torch.manual_seed(seed)

    _, image_size_x, image_size_y = init_im.shape

    image_size = image_size_x

    # randomly choose top-left corner of the patch
    top_left_x = torch.randint(0, image_size - patch_size, (1,)).item()
    top_left_y = torch.randint(0, image_size - patch_size, (1,)).item()

    patch_noise = torch.randn(1, patch_size, patch_size, device=init_im.device) * sig

    init_im[:, top_left_x:top_left_x + patch_size, top_left_y:top_left_y + patch_size] += patch_noise

    return init_im

def local_removal(init_im, patch_size, seed=None):
    '''
    Adds a localized occlusion (mask) to the image.
    - init_im (torch.Tensor): initial image
    - patch_size (int): size of the square patch where occlusion is applied
    '''
    ...
    _, image_size_x, image_size_y = init_im.shape
    top_left_x = torch.randint(0, image_size_x - patch_size, (1,)).item()
    top_left_y = torch.randint(0, image_size_y - patch_size, (1,)).item()

    # Apply occlusion (set the patch to zero)
    init_im[:, top_left_x:top_left_x + patch_size, top_left_y:top_left_y + patch_size] = 0
