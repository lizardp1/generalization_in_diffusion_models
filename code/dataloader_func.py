...
def split_dataset(source_dir, train_dir, test_dir, train_test_split_ratio=0.8, seed=None):
    ...
    split_index = int(train_test_split_ratio * total_images)

    train_images = all_images[:split_index]
    test_images = all_images[split_index:]

    for image in train_images:
        os.rename(os.path.join(source_dir, image), os.path.join(train_dir, image))

    for image in test_images:
        os.rename(os.path.join(source_dir, image), os.path.join(test_dir, image))
...
######################################################################################
def prep_celeba_orig(all_images,k=None, mean_zero=False):
    '''
...
def prep_celeba(all_images, k=None, mean_zero=False):
    '''
    Prepares CelebA dataset by converting images to grayscale, normalizing, and replicating with different intensities.
    @k: number of replicas of each image with a different intensity
    '''
    all_images_tensor = {}

    print(all_images['train'].shape)
    print(all_images['test'].shape)

    # Process train images
    train_images = int_to_float(all_images['train'])
    #print(f"Train images after int_to_float: {train_images.shape}")
    
    train_images = rgb_to_gray(train_images)  # Convert to grayscale
    #print(f"Train images after rgb_to_gray: {train_images.shape}")
    ...
    # Process test images
    test_images = int_to_float(all_images['test'])
    #print(f"Test images after int_to_float: {test_images.shape}")
    
    test_images = rgb_to_gray(test_images)
    #print(f"Test images after rgb_to_gray: {test_images.shape}")
...
def rgb_to_gray(data):
    # Check if the input has 4 dimensions (n, h, w, c)
    if len(data.shape) == 4:
        n, h, w, c = data.shape
        return data.mean(3).reshape(n, h, w, 1)  # Convert RGB to grayscale
    elif len(data.shape) == 3:
        # If already grayscale (n, h, w), just reshape to add the channel dimension
        n, h, w = data.shape
        return data.reshape(n, h, w, 1)
    else:
        raise ValueError(f"Expected 3 or 4 dimensions but got {data.shape}")
