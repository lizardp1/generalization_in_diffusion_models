...
from trainer_3 import run_training
...
def prep_data_swap(train_coeffs,args, save):
    ...
    if args.swap is False:
        ...
        test_set = train_coeffs[-1000::] #for these experiments, take test image from end of train set
    else:
    ...
    if save: 
        torch.save(train_set, args.dir_name + '/train_set_gmm_samples.pt')
        torch.save(test_set, args.dir_name + '/test_set_gmm_samples.pt')
...
def repeat_images(train_set,args, N_total): 
    ...
    train_set = torch.tile(train_set, (n,1,1))  # Modified tiling for point samples
    return train_set
...
def main():
    ...
    parser.add_argument('--num_channels', default=2, help='dimension of the GMM samples')  # Changed to 2 for 2D GMM
    parser.add_argument('--num_points', type=int, default=4, help='Number of points per GMM sample (4 for 2D, 9 for 3D)')
    ...
    parser.add_argument('--num_epochs', type=int, default=500) ##changed this from 1000 for speed to test
    ...
    parser.add_argument('--SLURM_ARRAY_TASK_ID',type=int) ##make this 2 to test
    ...
    ### load raw data
    train_set = torch.load(args.data_path + 'gmm_100000_samples_2d.pt') #change from train_no_repeats_80x80.pt to train80x80_no_repeats.pt to toy_dataset_gmm_80x80.pt
    
    print('all data: ', train_set.size() )    
    if int(train_set.shape[0]) < args.set_size: 
    ...
    train_set, test_set = prep_data_swap(train_set, args, save=True)    
    print('train: ', train_set.size(), 'test: ', test_set.size() )
    args.num_channels = train_set.shape[2] # Will be dimension of GMM samples
    ...
    if args.debug:
        ...
        ### repeat train images 
        train_set = repeat_images(train_set, args, N_total=200000)
