...
def im_set_corr_orig(set1, set2, remove_mean=True):
    '''
    im_set: tensor of size N,C,H, W
    '''
...
def remove_repeats_orig(dataset, threshold=.95): 
...
def remove_repeats(dataset, threshold=.95, chunk_size=1000, batch_size=100):
    '''
    Removes repeated images from the dataset based on correlation threshold.
    
    Args:
        dataset: The dataset tensor of shape (N, C, H, W).
        threshold: The correlation threshold for removing similar images.
        chunk_size: The size of each chunk for memory-efficient processing.
        batch_size: The size of smaller batches inside each chunk for memory-efficient correlation calculation.
        
    Returns:
        A cleaned dataset with repeated images removed.
    '''
    ...
    # Downsample the dataset to 20x20 images to save computation
    pool = torch.nn.AvgPool2d(int(dataset.shape[2] / 20))
    dataset_down = pool(dataset).half()  # Convert to float16 to reduce memory
    
    # Initialize a mask to keep track of unique images
    keep_mask = torch.ones(dataset.shape[0], dtype=torch.bool)
    
    num_chunks = dataset_down.shape[0] // chunk_size + (dataset_down.shape[0] % chunk_size != 0)
    
    for i in range(num_chunks):
        start_i, end_i = i * chunk_size, min((i + 1) * chunk_size, dataset_down.shape[0])
        set1 = dataset_down[start_i:end_i]

        # Compute correlation batch-wise within the chunk
        for j in range(0, set1.shape[0], batch_size):
            set1_batch = set1[j:j+batch_size]
            
            for k in range(j, set1.shape[0], batch_size):
                set2_batch = set1[k:k+batch_size]
                
                corrs = im_set_corr(remove_im_mean(set1_batch), remove_im_mean(set2_batch))
                
                if j == k:
                    corr_data_diag = torch.tril(corrs, diagonal=-1)
                else:
                    corr_data_diag = corrs

                rep_IDs_1, rep_IDs_2 = torch.where(abs(corr_data_diag) > threshold)

                rep_IDs_2_global = rep_IDs_2 + start_i + k
                keep_mask[rep_IDs_2_global] = False
    
    # Filter out repeated images
    data_cleaned = dataset[keep_mask]
    
    return data_cleaned

def im_set_corr(set1, set2, remove_mean=True):
    '''
    Computes the correlation between two sets of images.
    
    Args:
        set1: Tensor of shape (N, C, H, W).
        set2: Tensor of shape (M, C, H, W).
        remove_mean: Whether to remove the mean from each image.
        
    Returns:
        A correlation matrix of shape (N, M).
    '''
    ...
    norms1 = set1.norm(dim=(2, 3), keepdim=True).norm(dim=1, keepdim=True)
    norms1[norms1 == 0] = 0.001  # Avoid dividing by zero for blank images
    norms2 = set2.norm(dim=(2, 3), keepdim=True).norm(dim=1, keepdim=True)
    norms2[norms2 == 0] = 0.001  # Avoid dividing by zero for blank images
    
    return torch.matmul((set1 / norms1).flatten(start_dim=1), (set2 / norms2).flatten(start_dim=1).T)
