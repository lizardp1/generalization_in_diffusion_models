...
def one_iter(model, batch, criterion, args):
    ...
    # Add noise
    noisy, noise, noise_levels = add_noise(clean, args.noise_level_range, args.device)
...
def train_epoch(model, trainloader, criterion, optimizer, args):
    ...
    return model, loss_sum/(i+1), l2_dist_sum/(i+1)
...
def run_training(model, trainloader, testloader, criterion, optimizer, args):
    ...
    for epoch in range(args.num_epochs):
        ...
        # Train
        model, train_loss, train_l2 = train_epoch(model, trainloader, criterion, optimizer, args)
